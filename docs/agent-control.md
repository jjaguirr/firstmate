...
| `relaunch` | Replace a running or positively stopped agent in the same endpoint and worktree, or reattach a positively missing ordinary-worker endpoint by creating one replacement endpoint in its exact recorded worktree. The adapter and harness/profile stay recorded unless explicitly selected. | The new agent is alive on its recorded endpoint, and the durable record names the harness that is actually running. A missing-endpoint reattach retains the task's exact Treehouse lease and worktree. |
...
4. **Classify the old endpoint.**
   An `alive` or `dead` endpoint follows the existing `exit` path, preserving its endpoint for the replacement.
   A `missing` endpoint is recoverable only for an ordinary ship or scout task whose metadata has exactly one durable Treehouse lease matching the current home, task id, project, and recorded worktree.
   The lease id, holder, and canonical home are all required; a label, branch, or pathname never establishes ownership.
5. **Launch the replacement** through its single owner, `bin/fm-spawn.sh --relaunch`.
   A dead endpoint is adopted as before.
   A missing endpoint receives one new endpoint only in the recorded tmux or Herdr session and only with its shell bound to the recorded worktree.
   The launch owner clears the previous harness's per-task wiring and arms a fresh busy generation.
...
- A launch failure **after** the agent is stopped, or while a missing endpoint is being recreated, restores the prior durable record, keeps the progress note so a later recovery still has it, marks the journal `failed:launching`, and reports plainly that no agent is running and where the work is preserved.
...
All nine verified harness launch templates receive the same already-bound `WT`, including Cursor's exact `--workspace` argument, so the endpoint recovery difference is backend-only rather than a harness-specific reattach path.
...
- `tests/fm-control-relaunch.test.sh` - the relaunch transaction: identity preservation, harness switching, the progress note, checkpoint refusals, missing-endpoint clean and dirty worktree preservation, foreign and absent-worktree refusal, direct spawn recovery, and rollback after a failed launch.

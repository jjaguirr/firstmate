...
The compatibility helper `fm_backend_agent_alive` continues to collapse those detailed results to `alive`, `dead`, or `unknown` for every caller that needs only that three-state view, including the watcher's wedge-escalation liveness reading.
...
FM_QUOTA_SCAN_INTERVAL=300   # seconds between provider quota-refusal scans on the watcher poll; that cadence is the only entry to bin/fm-quota-watch.sh, whose contract (evidence, provider attribution, wait records, one resume per reset) is owned by bin/fm-quota-lib.sh
FM_QUOTA_PROBE_TIMEOUT=20    # seconds allowed per quota-axi call; a read that does not finish is missing evidence, never headroom
...
FM_QUOTA_NOTICE_RESET_MAX_SECS=21600 # maximum seconds after detection that a RESET TIME read from a RENDERED LIMIT NOTICE may state, with FM_QUOTA_RESET_GRACE_SECS added after it to reach the wait's deadline; a reset the notice states below that ceiling governs the wait, one above it is truncated to the ceiling, and a reset the vendor's own accounting stated is never truncated
...
FM_CREW_STATE_RUNS_LIMIT=200  # recent no-mistakes run rows scanned when selecting the run that owns a crew's branch
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates, and the interval at which an already-classified stale hash re-reads authoritative crew state instead of escalating a wait as a wedge; a pane backed off by FM_WEDGE_AFFIRMATIVE_BACKOFF_COUNT below keeps this read interval but wakes on the longer FM_PAUSE_RESURFACE_SECS cadence; stale panes whose crew is not provably working surface immediately unless they declare the pause verb, or their reconciled state is parked at a decision or done, which surfaces once and then takes the FM_PAUSE_RESURFACE_SECS recheck below
...
FM_PAUSE_RESURFACE_SECS=3600       # seconds before the watcher re-surfaces a declared external wait or verified captain-held transfer for a recheck, including a live busy pane past FM_BUSY_TURN_MAX_SECS, the cadence an already-surfaced reconciled parked or done pane rechecks on, the cadence a recorded provider quota wait (FM_QUOTA_* above) rechecks on, and the cadence an idle-stale pane backed off by FM_WEDGE_AFFIRMATIVE_BACKOFF_COUNT below wakes on; the away-mode daemon uses the same setting for all of those
FM_WEDGE_DEMAND_INSPECT_COUNT=3    # unexplained provably-working stale escalations on the same unchanged pane before demand-deep-inspection is added; the repetition fallback only, since a pane with no agent alive at its recorded endpoint carries the marker on its first escalation and one reading alive never counts toward this threshold on the idle-stale path (a pane past the busy-turn bound keeps this threshold whatever the endpoint reads)
FM_WEDGE_AFFIRMATIVE_BACKOFF_COUNT=3   # consecutive machine-verified alive endpoint readings on the idle-stale path before that pane's wake moves from FM_STALE_ESCALATE_SECS onto the bounded FM_PAUSE_RESURFACE_SECS cadence; the liveness read itself stays on the short cadence, the first reading that is not alive returns the pane to it, and a pane past FM_BUSY_TURN_MAX_SECS is never backed off because an alive reading is not evidence there

...
FM_WEDGE_DEMAND_INSPECT_COUNT=3    # consecutive provably-working stale escalations on the same unchanged pane before demand-deep-inspection is added; the repetition fallback only, since a pane with no agent alive at its recorded endpoint carries the marker on its first escalation and one reading alive never counts toward this threshold

...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates, and the interval at which an already-classified stale hash re-reads authoritative crew state instead of escalating a wait as a wedge; stale panes whose crew is not provably working surface immediately unless they declare the pause verb, or their reconciled state is parked at a decision or done, which surfaces once and then takes the FM_PAUSE_RESURFACE_SECS recheck below
...
FM_PAUSE_RESURFACE_SECS=3600       # seconds before the watcher re-surfaces a declared external wait or verified captain-held transfer for a recheck, including a live busy pane past FM_BUSY_TURN_MAX_SECS, and the cadence an already-surfaced reconciled parked or done pane rechecks on; the away-mode daemon uses the same setting for a declared external wait or verified captain-held transfer

...
FM_PAUSE_RESURFACE_SECS=3600       # seconds before the watcher re-surfaces a declared external wait or verified captain-held transfer for a recheck, including a live busy pane past FM_BUSY_TURN_MAX_SECS, the cadence an already-surfaced reconciled parked or done pane rechecks on, and the cadence an idle-stale pane backed off by FM_WEDGE_AFFIRMATIVE_BACKOFF_COUNT below wakes on; the away-mode daemon uses the same setting for a declared external wait or verified captain-held transfer

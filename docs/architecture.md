...
One branch can hold several runs, so the newest attributable run owns it, and a run head this local copy does not have yet is reported as an explicit unknown rather than as a mismatch that would let an older dead run be claimed instead.

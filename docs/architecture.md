...
A crew never has to declare the two waits `bin/fm-crew-state.sh` can reconcile on its own - `parked` at a decision, and `done` awaiting landing - and those panes are idle by design for the same reason a declared wait is, so they get the same treatment on the same terms.

...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose reconciled state does not prove that the idleness is expected, first sightings of a reconciled expected-idle state, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
Every escalation reports that count in the same wake-reason grammar the away-mode daemon escalates on, and only an escalation the liveness evidence leaves unexplained advances it.
That marker is driven by the agent-liveness reading taken in the branch that is about to escalate, and falls back to repetition only when that reading is unavailable: no agent alive at the recorded endpoint carries the marker on the first escalation, an endpoint reading alive never counts toward the repetition threshold on the idle-stale path, and an ambiguous, unreadable, or never-probed endpoint keeps the repetition threshold unchanged.
A pane past the busy-turn bound keeps the repetition threshold whatever the endpoint reads, because a busy pane is already known to be running an agent and process liveness cannot see the hung foreground call that bound exists to catch; a dead or authoritatively missing endpoint still carries the marker there on its first escalation.
Every one of those still queues the same stale wake on the same cadence, so the reading decides how loudly a pane is reported and never whether it is reported, until that same reading has come back affirmative often enough to earn the cadence backoff described next.
Once the idle-stale path has read the endpoint alive `FM_WEDGE_AFFIRMATIVE_BACKOFF_COUNT` times in a row, that pane keeps the same liveness read every `FM_STALE_ESCALATE_SECS` but moves its wake onto the bounded `FM_PAUSE_RESURFACE_SECS` cadence, because a fourth identical affirmative reading tells a supervisor nothing the first three did not.
The first reading that is not an admitted alive - dead, or any ambiguous, unreadable, or unverified answer - ends that backoff outright and returns the pane to the short cadence with no credit carried over, so a pane whose agent dies while backed off is caught on the next read rather than at the end of the long window.
The backoff never applies where an alive reading is refused as evidence, so a pane past the busy-turn bound is never backed off; the accepted cost of the backoff, and why it was accepted, is recorded beside `FM_WEDGE_AFFIRMATIVE_BACKOFF_COUNT` in `bin/fm-watch.sh`.
...
A pane whose worker is waiting out a recorded provider quota refusal is absorbed before that liveness read is taken at all, on the same bounded `FM_PAUSE_RESURFACE_SECS` cadence, with a reason naming the provider, the reset time only where one could be read, and whether that wait is resumed automatically, rather than a wedge.
Reaching that branch first is deliberate: a quota-refused worker's endpoint reads alive, so leaving it until afterwards would spend an affirmative reading on a pane whose idleness is already explained and hand the affirmative-liveness backoff the job of deciding how often a known wait is rechecked.
The record carries its own deadline and retires itself once that deadline passes, so the absorb cannot outlive its evidence and a worker that does not resume is escalated by the unchanged wedge path.
`bin/fm-quota-lib.sh` owns what counts as evidence for that refusal, how a worker is attributed to a provider, and why at most one resume is ever spent per reset.
A busy pane is otherwise exempt from staleness, but only until its latest `state/<id>.turn-ended` marker reaches `FM_BUSY_TURN_MAX_SECS`, or its `state/<id>.meta` spawn record reaches that age before any turn completes; past that bound it is routed through the same wedge escalation, with the identical repetition reason, escalation count, worktree-write deferral, and `demand-deep-inspection` marker it has always had for an alive or unknown endpoint reading, which is why an alive reading is refused as evidence there, while a dead or authoritatively missing endpoint carries the marker at its first escalation there too - for inspection only - never an automatic interrupt, signal, or restart.
...
For an ordinary crew that has stopped, the normal-mode watcher first surfaces one stale wake, then applies that same cadence to an unchanged `paused:` or durable `captain-held` endpoint.
Authoritative recovery of the declared class still requires the backend to confidently report that crew's agent dead, so live or inconclusive liveness remains fail-open at that first surface.
That surface is spent once per declaration, not once per pane hash: while the same declaration stands, a later pane redraw is the same wait and keeps the bounded cadence, so a crew that finished and is waiting for its PR to be merged - authoritatively done rather than paused, with its agent still alive - cannot re-alarm every time its idle pane repaints.
What makes that true is a record of the declaration a surface was actually spent on, written only by the site that surfaces one and keyed to a digest of the declaring status line itself, never to the window alone.
Because the record names the declaration, a different one under the same window - a `paused:` reason replaced by another, or a pause becoming a `captain-held` transfer - is a new event that still earns its own surface, and because only the surfacing site writes it, a declaration the watcher merely admitted to the bounded cadence without surfacing - the busy pane past its completed-turn bound, whose declared wait is honoured before it ever goes quiet - still surfaces once when that pane goes idle rather than waiting out the long cadence in silence.
Only the declaration ending, or an authoritative working verdict, returns that pane to ordinary stale and wedge handling, which is what keeps an undeclared wedge surfacing on every fresh stale hash.
A crew never has to declare the two waits `bin/fm-crew-state.sh` can reconcile on its own - `parked` at a decision, and `done` with its work finished - and those panes are idle by design for the same reason a declared wait is, so they get the same treatment on the same terms.
Each distinct reconciled state surfaces once, keyed to a digest of the state, the standing status line, and that log's own modification time, and only its later redraws take the bounded cadence; a crew that resumes and parks again, or moves from `parked` to `done`, is a new state that earns its own surface.
The absorb keeps the wedge timer armed rather than dropping it, so a pane that stops being parked or done while it stays frozen escalates as the wedge it has become, and an absorbed pane costs one current-state read per `FM_STALE_ESCALATE_SECS` instead of one per poll.
A pane rendering busy ends that expected-idle stretch outright, because a reconciled state is read from the crew rather than declared by it, so the crew working is the whole of the evidence and the next park earns its own surface even when a run owns the branch and the status log gains no entry between the two.
A merely changed hash on a still-idle pane is not that evidence and keeps the stretch, which is what stops a repainting pane from re-surfacing.
The `done` recheck states only that the work is finished and asks whether it landed, because `bin/fm-crew-state.sh` maps a green PR and an already-merged one onto the same `done` token.
...
Fresh stale panes use the same current-state read before trusting the status log, so an active run or a proven busy worker outranks an old captain-relevant status-log line left behind before validation, and a reconciled `parked` or `done` state stops the log's already-reported wait from being presented again on every redraw.
An already-classified stale hash re-reads that current state only when its wedge timer reaches `FM_STALE_ESCALATE_SECS`, so a run that parks at a gate or goes checks-green behind an unchanged pane cannot escalate as a wedge.
...
The same poll runs `bin/fm-quota-watch.sh` on its own bounded cadence, which records a quota refusal as a bounded wait and delivers the single resume once that wait's reset arrives.
That is the one place in the poll loop that may deliver text to a worker, and it does so only after the endpoint liveness read, the semantic busy contract, and `bin/fm-crew-state.sh` all agree the worker is parked; anything unreadable or contradictory refuses the resume and returns the pane to ordinary escalation.
It is delivered from the poll loop rather than from a supervision turn because the incident it exists for parked every worker and mate on one account at once, which parks any recovery that needs a model turn along with them.
...
Because of that, a per-wake read of only the latest line can bury an earlier still-open `needs-decision`/`blocked` under later unrelated appends; `fm-wake-drain.sh` prints a separate, fleet-wide OPEN DECISIONS section on every presentation (including the empty-queue path session-start relies on), built through `fm-classify-lib.sh`'s cursor-backed incremental scan and its shared task answerability verdict: a buried decision keeps surfacing until it is explicitly resolved or an authoritative active run-step plus a later progress line stating that same key prove that one key was superseded, while a decision raised mid-run or followed only by unrelated progress lines stays open and each presentation folds only new status-log appends.
That verdict has exactly one definition, in `fm-classify-lib.sh`: `bin/fm-send.sh`'s `--resolve-key`, the `bin/fm-afk-return.sh` blocker gate, and `bin/fm-fleet-snapshot.sh`'s triage hints read it rather than reimplementing it, so no surface can present a key as waiting that another then refuses as unanswerable.
The triage view adds exactly one rule of its own, because a completed scout or ship task has already delivered its report or PR: that task's stale records surface as the report pointer rather than as a reopened pending decision.
...
A third bounded section, RECORD DIVERGENCE, prints on the same drains for the opposite failure: the durable status fold - never the answerability verdict, so an active run's supersession cannot masquerade as one - went quiet on a key that the durable captain-held task still shows as open, so the status side reads as complete while the two records contradict each other; `bin/fm-captain-hold.sh diverged` decides what counts and closes nothing, and `docs/captain-hold-lifecycle.md` owns the mechanism.
...
The always-on watcher also uses that library's absorb classification before status-log terminality is trusted, on the bounded set of sites named above rather than on every wake, while the daemon maintains distinct wedge and declared-wait recheck cadences and no reconciled expected-idle absorb of its own.

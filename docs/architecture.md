...
A pane whose worker is waiting out a recorded provider quota refusal is absorbed before that liveness read is taken at all, on the same bounded `FM_PAUSE_RESURFACE_SECS` cadence, with a reason naming the provider and the reset time rather than a wedge.

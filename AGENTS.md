...
state/               runtime records and signals; gitignored
  ...
  <id>.quota-nudged.lock  the single-flight hold two concurrent scans claim one reset through, so a scan overlapping its own next tick cannot deliver a second nudge; released as soon as the claim is made, reclaimed from a dead holder by the next scan, removed by teardown
  .quota-scan .quota-probe.json .quota-probe.json.providers .quota-catalog.json .quota-probe-resume.*  the per-home scan cadence marker, cached quota-axi evidence and the provider set it was fetched for, and the scan-scoped snapshot the resumes in one scan read their accounts from, one fresh read per provider; safe to delete
  ...
  .<id>.open-decisions-cursor  per-task byte cursor carrying both the folded open-decision set and the same-key-progress witness set the answerability verdict needs, bounding the OPEN DECISIONS scan's cost to new status-log appends; written only by fm-classify-lib.sh's status_open_decisions_incremental, removed by teardown, safe to delete (forces one full re-fold)
  ...
  .hash-* .count-* .stale-* .stale-since-* .paused-* .reconciled-* .wedge-escalations-* .wedge-affirmative-* .writing-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak   watcher internals; never touch
...
Judge validation by the attributed run step through `bin/fm-crew-state.sh`, not by shell liveness or the last status event.
...
A teardown refusal for uncommitted or unlanded work is a stop-and-investigate result, never an obstacle to bypass, and some teardown refusals are not cleared by `--force` even with discard authority - `bin/fm-teardown.sh` owns the exact refusal classes.
...
3. For `check:`, act on the named poll result, including merges, Relay events, provider usage limits, and process-to-event source results.

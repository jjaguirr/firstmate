...
#     fm_nm_head_matches_worktree, the shared rule) both match this worktree,
#     then runs `no-mistakes axi abort --run <id>` for that verified run
...
cleanup_firstmate_home_children() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    rm -f "$sub_state/$child_id.turn-ended" \
      "$sub_state/$child_id.quota-wait" "$sub_state/$child_id.quota-resurfaced" \
      "$sub_state/$child_id.quota-nudged" "$sub_state/$child_id.quota-spent" \

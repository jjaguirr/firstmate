...
# fm_backend_tmux_session_exists: true only when <session> is present by exact
# name in a readable server inventory. Recreating a vanished endpoint needs
# this: a session that is itself gone must refuse, never license inventing
# another session for the task.
...
# window id `new-window -P` returned, never by a name another window in the
# session could share. It exists for one caller: aborting a replacement window
# that a missing-endpoint recovery created but never published, where closing
# the wrong window would destroy a live worker's terminal.

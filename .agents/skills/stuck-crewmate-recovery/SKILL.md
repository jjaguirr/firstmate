...
A crew whose own run is still running is NOT reported this way even while its copy is behind the pipeline's pushed commits - it keeps its real state and gate detail - so this read means a run that had already concluded could not be confirmed, and no run head this copy cannot read is ever reported as failed.
